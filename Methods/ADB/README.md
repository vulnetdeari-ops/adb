...
ADB is a product method for AI coding agents. Bubby sets where the product must land. Roles follow the project’s `AGENTS.md` (setup copy): Lead only directs; Plan, Code and Review do the work. It is not an agent framework, a fake company, or a pile of extra process.
...
- **Build** — Code executes against `adb/` (Lead directs). Prove on a real path (browser, tests, logins). Missing required behavior is fixed or written down, not ignored. Git: AGENTS.md (auto-commit when the job is done; push only when Bubby asks).
