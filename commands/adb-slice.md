...
Inherit global `AGENTS.md`. If DEFINE was not approved, stop and run `/adb-define`. Lead directs; Lead does not implement. Never spawn — queue on living **Plan** / **Code** / **Review**.
...
2. **Brief** — goal, path, `adb/` pointers, scope, proof (AGENTS.md Work). Heavy: **Plan** writes plan + done criteria before Code implements. Do not skip Plan.
3. **Loop** — SPEC → BUILD → PROVE → RECORD. Code builds and proves. PROVE fail → do not RECORD as done. Fix-round cap 3, then Bubby (not `CARRIED`). Git: AGENTS.md — Code commits automatically when the job is done. Push only when Bubby asks.
