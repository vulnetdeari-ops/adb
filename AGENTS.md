...
When the owner is in this chat, **ask Q1 (language) with the letters**. Do not run `setup-into-project.sh` as a substitute — that path does not ask language and does not write `OWNER.md` / `LESEN.html`. Do not treat this factory file as the app’s `AGENTS.md`.
